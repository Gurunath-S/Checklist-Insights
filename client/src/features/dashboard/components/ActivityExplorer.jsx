...
import { getActivityTemplatesApi, getActivityDatesApi } from '../services/dashboardService';
import { getDateResponsesApi } from '../../reports/services/reportsService';
...
const TemplateList = ({ userId, onSelect, startDate, endDate }) => {
  ...
  useEffect(() => {
    ...
    getActivityTemplatesApi(userId, params)
      .then(data => {
...
const DateList = ({ userId, template, onSelect, startDate, endDate }) => {
  ...
  useEffect(() => {
    ...
    getActivityDatesApi(userId, template.template_id, params)
      .then(data => {
        setDates(data.data || []);
        setTotalPages(data.totalPages || 1);
      })
...
const ResponseDetail = ({ userId, template, dateRow }) => {
  ...
  useEffect(() => {
    getDateResponsesApi(userId, template.template_id, dateParam)
