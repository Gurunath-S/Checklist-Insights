...
// ─── Activity Explorer Endpoints ────────────────────────────────────────────

// Level 1: All templates the user has submitted responses for
app.get('/api/activity/templates/:userId', authenticateToken, async (req, res) => {
  const userId = parseInt(req.params.userId);
...
// Shows created_at date; if selected_date differs, also returns it
app.get('/api/activity/dates/:userId/:templateId', authenticateToken, async (req, res) => {
  const userId = parseInt(req.params.userId);
  ...
  try {
    ...
    `;
    res.json(rows.map(r => ({
      submitted_day: r.submitted_day,            // date the user actually hit submit
      selected_date: r.selected_date,            // date they chose as the "report for" date
...
app.get('/api/activity/responses/:userId/:templateId/:date', authenticateToken, async (req, res) => {
  const userId = parseInt(req.params.userId);
  const templateId = parseInt(req.params.templateId);
